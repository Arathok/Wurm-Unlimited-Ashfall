...
public class MountFlaskOfVynoraPerformer implements ActionPerformer {

    private final Method mMethodsItemsTake;

    public MountFlaskOfVynoraPerformer() {
        try {
    ...
    public boolean action(Action action, Creature performer, Item target, short num, float counter) {
        ...
        try {
            ...
            FlaskOfVynora.setEmbarking(target, true);

            try {
                target.getParent().dropItem(target.getWurmId(), false);
            } catch (NoSuchItemException e) {
                Ashfall.logger.log(Level.WARNING,"Huh broom has no parent?", e);
            }
            ...
            Ashfall.logger.log(Level.WARNING,String.format("Error mounting broom %d by %s", target.getWurmId(), performer.getName()), e);
